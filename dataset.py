...
class SetThreshold(torch.utils.data.Dataset):
    ...
    def __init__(self, args):
        ...
        self.training_subset = random.sample(training_images, k=15)
        self.testing_images = []
        # exclusion of the training images for the selection of the second element
        to_exclude = set(training_images)
        for i in range(len(self.training_subset)):
            while True:
                candidate = random.choice(list_images)
                if candidate not in to_exclude:
                    self.testing_images.append(candidate)
                    to_exclude.add(candidate)
                    break
        self.images = self.training_subset + self.testing_images
    ...
    def __getitem__(self, item):
        ...
        return self.transform(image), self.images[item] in self.training_subset
